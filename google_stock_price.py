...
class Recurrent_neural_network:
    ...
    def logGenerator(self):
        ...
        tItns = [100, 200]
        tActFuns = [ "Relu","TanH"]
        tLRs = [0.01, 0.05, 0.001]
        tTrain = [0.80, 0.90]
        tNeurons = [10, 12]

        #Storing result and error data in the following
        trainMSE = testMSE = list()
        trainRMSE = testRMSE = list()
        trainMAE = testMAE = list()
        trainR2 = testR2 = list()
                ...
                for iterations in tItns:
                    ...
                    for neurons in tNeurons:
                        ...
                        for split in tTrain:
                            ...
                            #Mean squared errors
                            trainMSE.append(mean_squared_error(self.yTrain, self.trainResults))
                            testMSE.append(mean_squared_error(self.yTest, self.testResults))

                            #Root mean squared errors
                            trainRMSE.append(sqrt(mean_squared_error(self.yTrain, self.trainResults)))
                            testRMSE.append(sqrt(mean_squared_error(self.yTest, self.testResults)))

                            #Mean absolute errors
                            trainMAE.append(mean_absolute_error(self.yTrain, self.trainResults))
                            testMAE.append(mean_absolute_error(self.yTest, self.testResults))

                            #R2 scores
                            trainR2.append(r2_score(self.yTrain, self.trainResults))
                            testR2.append(r2_score(self.yTest, self.testResults))

        # Make a table for exporting
        logDataTable = pandas.DataFrame({
                    'Hidden_Neurons':neurons_list,
                    'MSE_Test':testMSE,
                    'MAE_Train':testMAE,
                    'MAE_Test':testMAE,
